...
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate, NSMenuItemValidation, NSMenuDelegate {
    ...
    private typealias TilingShortcutBinding = (action: KeyboardShortcutSettings.Action, shortcut: StoredShortcut, whenClause: ShortcutWhenClause)
    private var configuredTilingShortcutBindings: [TilingShortcutBinding]?
    ...
    private func handleShortcutDefaultsDidChange() {
        configuredTilingShortcutBindings = nil
        clearConfiguredShortcutChordState()
        scheduleReloadConfigurationMenuItemRefresh()
        scheduleSplitButtonTooltipRefreshAcrossWorkspaces()
    }

    private func currentConfiguredTilingShortcutBindings() -> [TilingShortcutBinding] {
        if let configuredTilingShortcutBindings { return configuredTilingShortcutBindings }
        installShortcutDefaultsObserver()
        // Resolve defaults and decode saved bindings only after configuration changes.
        // Unbound tiling actions add no per-keystroke settings or context lookups.
        let bindings: [TilingShortcutBinding] = KeyboardShortcutSettings.tilingActions.compactMap { action in
            guard let shortcut = KeyboardShortcutSettings.shortcutIfBound(for: action) else { return nil }
            return (action, shortcut, KeyboardShortcutSettings.effectiveWhenClause(for: action))
        }
        configuredTilingShortcutBindings = bindings
        return bindings
    }
    ...
    private func currentConfiguredShortcutChordActions() -> [KeyboardShortcutSettings.Action] {
        KeyboardShortcutSettings.Action.allCases.filter { action in
            if action.tilingAction != nil {
                return currentConfiguredTilingShortcutBindings().contains {
                    $0.action == action && $0.shortcut.hasChord
                }
            }
    ...
    private func handleCustomShortcut(event: NSEvent) -> Bool {
...
#if DEBUG
            cmuxDebugLog("shortcut.action name=moveTabLeft \(debugShortcutRouteSnapshot(event: event))")
#endif
            _ = performMoveFocusedSurfaceShortcut(
                direction: .left,
                preferredWindow: event.window ?? NSApp.keyWindow ?? NSApp.mainWindow
            )
            return true
...
#if DEBUG
            cmuxDebugLog("shortcut.action name=moveTabRight \(debugShortcutRouteSnapshot(event: event))")
#endif
            _ = performMoveFocusedSurfaceShortcut(
                direction: .right,
                preferredWindow: event.window ?? NSApp.keyWindow ?? NSApp.mainWindow
...
#if DEBUG
            cmuxDebugLog("shortcut.action name=moveTabUp \(debugShortcutRouteSnapshot(event: event))")
#endif
            _ = performMoveFocusedSurfaceShortcut(
                direction: .up,
                preferredWindow: event.window ?? NSApp.keyWindow ?? NSApp.mainWindow
...
#if DEBUG
            cmuxDebugLog("shortcut.action name=moveTabDown \(debugShortcutRouteSnapshot(event: event))")
#endif
            _ = performMoveFocusedSurfaceShortcut(
                direction: .down,
                preferredWindow: event.window ?? NSApp.keyWindow ?? NSApp.mainWindow
        ...
        for binding in currentConfiguredTilingShortcutBindings() {
            if matchConfiguredShortcut(event: event, shortcut: binding.shortcut),
               binding.whenClause.evaluate(shortcutEventFocusContext(event).shortcutContext),
               let tilingAction = binding.action.tilingAction {
                let manager = preferredMainWindowContextForShortcutRouting(event: event)?.tabManager ?? tabManager
                _ = manager?.selectedWorkspace?.performTilingAction(tilingAction)
                return true
            }
        }
    ...
    private func executeConfiguredCmuxAction(
        ...
        func executeMoveFocusedSurface(_ direction: SplitDirection) -> Bool {
            guard let workspace = context.tabManager.selectedWorkspace,
            ...
            let didMove = moveSurfaceToNeighborOrSplit(
                panelId: panelId,
                workspace: workspace,
                direction: direction,
                focus: true,
        ...
        case .builtIn(let builtIn):
            ...
            case .splitDown:
                ...
                return didSplit
            case .moveTabLeft:
                return executeMoveFocusedSurface(.left)
            case .moveTabRight:
                return executeMoveFocusedSurface(.right)
            case .moveTabUp:
                return executeMoveFocusedSurface(.up)
            case .moveTabDown:
                return executeMoveFocusedSurface(.down)
            }
...
private extension NSWindow {
    ...
    @objc func cmux_performKeyEquivalent(with event: NSEvent) -> Bool {
        ...
        if let firstResponderWebView,
           shouldRouteBrowserCommandEquivalentThroughWebContentFirst(
               event,
...
#if DEBUG
            cmuxDebugLog(
                "  → browser command preflight " +
                (result ? "resolved before window menu path" : "left unclaimed; suppressing replay")
            )
#endif
            // The focused web view has already received this browser command once.
