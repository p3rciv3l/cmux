...
struct ContentView: View {
    ...
    private func commandPaletteConfigActionID(for commandId: String) -> String? {
        ...
        case "palette.moveTabLeft":
            return CmuxSurfaceTabBarBuiltInAction.moveTabLeft.configID
        case "palette.moveTabRight":
            return CmuxSurfaceTabBarBuiltInAction.moveTabRight.configID
        case "palette.moveTabUp":
            return CmuxSurfaceTabBarBuiltInAction.moveTabUp.configID
        case "palette.moveTabDown":
            return CmuxSurfaceTabBarBuiltInAction.moveTabDown.configID
        default:
            return nil
        }
    }
    ...
    private func commandPaletteCommandContributions() -> [CommandPaletteCommandContribution] {
        ...
        contributions.append(contentsOf: KeyboardShortcutSettings.tilingActions.map { action in
            CommandPaletteCommandContribution(
                commandId: action.tilingCommandID,
                title: constant(action.label),
                subtitle: workspaceSubtitle,
                keywords: ["tiling", "tile", "dwm", "master", "stack", "monocle", "layout", action.rawValue],
                when: { $0.bool(CommandPaletteContextKeys.hasFocusedPanel) }
            )
        })
        contributions.append(
            CommandPaletteCommandContribution(
                commandId: "palette.moveTabLeft",
                title: constant(String(localized: "command.moveTabLeft.title", defaultValue: "Move Tab Left")),
                subtitle: workspaceSubtitle,
        ...
        contributions.append(
            CommandPaletteCommandContribution(
                commandId: "palette.moveTabRight",
                title: constant(String(localized: "command.moveTabRight.title", defaultValue: "Move Tab Right")),
                subtitle: workspaceSubtitle,
        ...
        contributions.append(
            CommandPaletteCommandContribution(
                commandId: "palette.moveTabUp",
                title: constant(String(localized: "command.moveTabUp.title", defaultValue: "Move Tab Up")),
                subtitle: workspaceSubtitle,
        ...
        contributions.append(
            CommandPaletteCommandContribution(
                commandId: "palette.moveTabDown",
                title: constant(String(localized: "command.moveTabDown.title", defaultValue: "Move Tab Down")),
                subtitle: workspaceSubtitle,
    ...
    private func registerCommandPaletteHandlers(_ registry: inout CommandPaletteHandlerRegistry) {
        ...
        for action in KeyboardShortcutSettings.tilingActions {
            registry.register(commandId: action.tilingCommandID) {
                guard let tilingAction = action.tilingAction else { return }
                _ = tabManager.selectedWorkspace?.performTilingAction(tilingAction)
            }
        }
        registry.register(commandId: "palette.moveTabLeft") {
            if !executeConfiguredAction(id: CmuxSurfaceTabBarBuiltInAction.moveTabLeft.configID) {
                _ = AppDelegate.shared?.performMoveFocusedSurfaceShortcut(direction: .left, preferredWindow: observedWindow)
            }
        }
        registry.register(commandId: "palette.moveTabRight") {
            if !executeConfiguredAction(id: CmuxSurfaceTabBarBuiltInAction.moveTabRight.configID) {
                _ = AppDelegate.shared?.performMoveFocusedSurfaceShortcut(direction: .right, preferredWindow: observedWindow)
            }
        }
        registry.register(commandId: "palette.moveTabUp") {
            if !executeConfiguredAction(id: CmuxSurfaceTabBarBuiltInAction.moveTabUp.configID) {
                _ = AppDelegate.shared?.performMoveFocusedSurfaceShortcut(direction: .up, preferredWindow: observedWindow)
            }
        }
        registry.register(commandId: "palette.moveTabDown") {
            if !executeConfiguredAction(id: CmuxSurfaceTabBarBuiltInAction.moveTabDown.configID) {
                _ = AppDelegate.shared?.performMoveFocusedSurfaceShortcut(direction: .down, preferredWindow: observedWindow)
            }
        }
