...
final class TerminalSurface: Identifiable, ObservableObject {
    ...
    /// Exercises the production size-log path without constructing a terminal surface.
    static func debugLogTerminalSizeForTesting(_ message: @autoclosure () -> String) {
        sizeLog(message())
    }

    private static let sizeLoggingEnabled =
        ProcessInfo.processInfo.environment["CMUX_UI_TEST_SPLIT_CLOSE_RIGHT_VISUAL"] == "1"

    private static func sizeLog(_ message: @autoclosure () -> String) {
        guard sizeLoggingEnabled else { return }
...
final class GhosttySurfaceScrollView: NSView {
    ...
    /// Request a terminal redraw after geometry updates. Callers inside native layout
    /// callbacks must disable AppKit flushing to avoid reentering whole-window layout.
    func refreshSurfaceNow(reason: String = "portal.refreshSurfaceNow", flushAppKitLayout: Bool = true) {
    ...
    func setVisibleInUI(_ visible: Bool, flushAppKitLayout: Bool = true) {
        ...
        if !visible {
            ...
            refreshSurfaceNow(reason: "setVisibleInUI", flushAppKitLayout: flushAppKitLayout)
...
#if DEBUG
        var countsWindowMoveBindings = false
        var windowMoveBindingCount = 0
#endif
...
#if DEBUG
    /// Opts one native host into callback diagnostics without enabling global instrumentation.
    static func debugEnableWindowMoveBindingCounts(in host: NSView) {
        (host as? HostContainerView)?.countsWindowMoveBindings = true
    }

    static func debugWindowMoveBindingCount(in host: NSView) -> Int? {
        (host as? HostContainerView)?.windowMoveBindingCount
    }
#endif
    ...
    func updateNSView(_ nsView: NSView, context: Context) {
        ...
        if let host = hostContainer {
            host.onDidMoveToWindow = { [weak host, weak hostedView, weak coordinator] in
                ...
                let reusesBinding = coordinator.lastBoundHostId == hostId &&
                    TerminalWindowPortalRegistry.canReuseBinding(
                ...
                if !reusesBinding {
#if DEBUG
                    if host.countsWindowMoveBindings {
                        host.windowMoveBindingCount &+= 1
                    }
#endif
                    ...
                    // Ancestor-only moves can leave the leaf's local revision
                    // unchanged. Reconcile window coordinates without rebinding
                    // or flushing ancestor layout; do not consume a later revision.
                    TerminalWindowPortalRegistry.synchronizeForAnchor(host, syncLayout: false)
                }
