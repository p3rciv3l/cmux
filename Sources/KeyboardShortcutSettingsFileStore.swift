...
final class CmuxSettingsFileStore {
    ...
    private func parseShortcutBindingValue(
        ...
        guard let configuration = CmuxSettings.StoredShortcut.decodeConfiguration(
            rawValue,
            allowBareFirstStroke: action.allowsBareFirstStroke
        ) else { return nil }
        let shortcut = StoredShortcut(configuration: configuration)
        // Parsing can run while the store is initializing. Keep action-specific
        // normalization here without consulting the UI recorder or global store.
