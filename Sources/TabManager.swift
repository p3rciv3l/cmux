...
class TabManager: ObservableObject {
    ...
    func closeWorkspace(_ workspace: Workspace, recordHistory: Bool = true) {
        ...
        if recordHistory,
           ...
           let index = tabs.firstIndex(where: { $0.id == workspace.id }) {
            ...
            let closedWorkspaceEntry = ClosedWorkspaceHistoryEntry(
                workspaceId: workspace.id,
                windowId: AppDelegate.shared?.windowId(for: self),
                workspaceIndex: index,
                snapshot: snapshot
            )
            let closedItemRecord = ClosedItemHistoryRecord(entry: .workspace(closedWorkspaceEntry))
            ClosedItemHistoryStore.shared.push(closedItemRecord)
            WorkspaceHistoryStore.shared.recordClosedWorkspace(
                closedWorkspaceEntry,
                closedHistoryRecordId: closedItemRecord.id
            )
        }
    ...
    func newSurface() {
        // The shortcut path should always focus the newly created surface.
    ...
    func restoreClosedWorkspace(_ entry: ClosedWorkspaceHistoryEntry) -> Bool {
        ...
        WorkspaceHistoryStore.shared.removeClosedWorkspaceRecord(workspaceId: entry.workspaceId)
        WorkspaceHistoryStore.shared.recordOpenWorkspace(
            workspace,
            windowId: AppDelegate.shared?.windowId(for: self),
            workspaceIndex: tabs.firstIndex(where: { $0.id == workspace.id }),
            touchExisting: true
        )
        return true
    }
